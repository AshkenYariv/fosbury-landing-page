    ...
    <footer className="border-t border-border bg-bg py-16">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid gap-10 sm:grid-cols-2 md:grid-cols-5">
          ...
          <FooterColumn column="product" title="Product" links={copy.footer.productLinks} />
          <FooterColumn column="company" title="Company" links={copy.footer.companyLinks} />
          <FooterColumn
            column="industries"
            title="Industries"
            links={[...copy.footer.forLinks, ...copy.footer.personaLinks]}
          />
        </div>
      ...
      <ul className="mt-4 space-y-2.5">
        {links.map((l) => (
          <li key={l.label}>
            <TrackedLink
              href={l.href}
              track={footerEvent(column, l.label)}
